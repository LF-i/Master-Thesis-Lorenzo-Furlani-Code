...
def train(model, train_loader, optimizer, device: str, 
    ...
    for batch_data in train_loader: # earlier: tqdm(train_loader)
        ...
        l = sharpe_loss(output, batch_y, batch_vol, transaction_cost, apply_turnover_reg)
        train_loss += l.item()
...
def evaluate(model, val_loader, device: str,
    ...
    l = sharpe_loss(preds, returns, vols, transaction_cost, apply_turnover_reg)
    val_loss = l.item()
...
def output_stats(model, loader, device, transaction_cost, apply_turnover_reg
    ...
    preds           = torch.zeros((total_size, model.dim_out))
    returns         = torch.zeros((total_size, model.dim_out))
    returns_orig    = torch.zeros((total_size, model.dim_out))
    vols            = torch.zeros((total_size, model.dim_out))
    ...
    with torch.no_grad():
        for batch_data in loader:
            ...
            preds[idx:idx+curr_batch_size]          = output.detach().cpu()
            returns[idx:idx+curr_batch_size]        = batch_y.detach().cpu()
            returns_orig[idx:idx+curr_batch_size]   = batch_y_orig.detach().cpu()
            vols[idx:idx+curr_batch_size]           = batch_vol.detach().cpu()

            idx += curr_batch_size

    loss = sharpe_loss(preds, returns, vols, transaction_cost, apply_turnover_reg)
...
def grid_search_iter(splitter, start, test_delta, device, 
    ...
    iteration_params['dim_out']     = batch_y.shape[-1]
