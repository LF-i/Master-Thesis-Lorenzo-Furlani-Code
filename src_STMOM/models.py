...
class Transformer(nn.Module):
    ...
    def forward(self, x): # (batchsize, timesteps, n_features, n_assets)
        ...
        x_enc = x_enc.flatten(start_dim=-3, end_dim=-2) # (batchsize, timesteps x n_assets, d_model)
        x_enc = self.encoder(x_enc) # (batchsize, timesteps x n_assets, d_model)

        ## Decoder
        x_dec = self.dec_embedding(x_dec, x_cat_dec) # (batchsize, 1, n_assets, d_model)
        x_dec = x_dec.flatten(start_dim=-3, end_dim=-2) # (batchsize, 1 x n_assets = n_assets, d_model)
        x = self.decoder(x_dec, x_enc) # (batchsize, n_assets, d_model)

        x = self.dense(x) # (batchsize, n_assets, 1)
        x = x.flatten(start_dim=-2) # (batchsize, n_assets)
        x = torch.tanh(x) # (batchsize, n_assets)

        return x # (batchsize, dim_out)
