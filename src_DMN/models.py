...
class Transformer(nn.Module):
    ...
    def forward(self, x): # (batchsize, timesteps, n_features, n_assets)
        ...
        x_enc = x_enc.permute(0,2,1,3) # (batchsize, n_assets, timesteps, d_model)
        x_enc = x_enc.flatten(start_dim=0, end_dim=-3) # (batchsize x n_assets, timesteps, d_model)        
        x_enc = self.encoder(x_enc) # (batchsize x n_assets, timesteps, d_model)

        ## Decoder
        x_dec = self.dec_embedding(x_dec, x_cat_dec) # (batchsize, 1, n_assets, d_model)
        x_dec = x_dec.permute(0,2,1,3) # (batchsize, n_assets, 1, d_model)
        x_dec = x_dec.flatten(start_dim=0, end_dim=-3) # (batchsize x n_assets, 1, d_model)
